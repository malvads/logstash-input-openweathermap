  ...
  module Filters
    ...
    class OpenWeatherMap < LogStash::Filters::Base
      ...
      def cache_key(lat, lon)
        CacheGenerator.generate_memcached_key(lat, lon)
      end
      ...
      def filter(event)
        ...
        cached_weather_data = @memcached_manager.fetch_cached_data(cache_key(lat, lon))
        ...
        if cached_weather_data.nil?
          ...
          @memcached_manager.cache_data(cache_key(lat, lon), cached_weather_data) if cached_weather_data
        end
